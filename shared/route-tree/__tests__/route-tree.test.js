...
describe('routeSetState', () => {
  ...
  it('throws when given a path that does\'t exist', () => {
    const startRouteState = routeNavigate(demoRouteDef, null, ['foo'])
    expect(() => {
      const newRouteState = routeSetState(demoRouteDef, startRouteState, ['foo', 'nonexistent'], {state: 'value'})
    }).toThrowError(InvalidRouteError)
  })
})
...
describe('routeClear', () => {
  ...
  it('bails out early if the path does not exist', () => {
    const newRouteState = routeClear(startRouteState, ['foo', 'bar'])
    expect(startRouteState).toEqual(startRouteState)
  })
})
...
describe('checkRouteState', () => {
  ...
  it('returns an error for a selected route missing a definition', () => {
    const routeState = routeNavigate(demoRouteDef, null, ['foo']).updateChild('foo', n => n.set('selected', 'nonexistent'))
  ...
  it('returns an error for a selected route missing a component', () => {
    const routeDef = new RouteDefNode({children: {}})
